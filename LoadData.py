...
# define the used landmarks according to the paper (20 landmarks out of 68)
landmarks_20 = [8, 17, 21, 22, 26, 30, 31, 35, 36, 37, 39, 41, 42, 44, 45, 46, 48, 51, 54, 57]
...
def crop_face(image, keypoints):
    ...
    num_inlier_landmarks = (np.all(keypoints > selected_face_box[:2], axis=1) & np.all(
        keypoints < (selected_face_box[:2] + selected_face_box[2:]), axis=1)).sum()
    ...
    our_landmarks = keypoints[landmarks_20]

    return resized_cropped_image, our_landmarks


# https://docs.activeloop.ai/datasets/lfpw-dataset
def process_and_save_data(hub_path, data_type):
    print("# Preprocessing operations on the", data_type, "data started!")
    landmarks_dict = {}
    ...
    image_ids = []
    for image_id, (image, keypoints) in tqdm(enumerate(dataloader), desc='Processing Images and Landmarks',
        ...
        if image is not None:
            ...
            for landmark_id, (x, y) in enumerate(keypoints):
                ...
                image = cv2.putText(image, f'{landmark_id}', (x, y), font, fontScale, color, thickness, cv2.LINE_AA)
